...
void FMC_SDRAM_WriteBuffer(uint8_t *pBuffer,uint32_t WriteAddr,uint32_t n)
{
	for(;n!=0;n--)
	{
		*(volatile uint32_t*)(Bank5_SDRAM_ADDR+WriteAddr)=*pBuffer;
		WriteAddr++;
		pBuffer++;
	}
}
void FMC_SDRAM_ReadBuffer(uint8_t *pBuffer,uint32_t ReadAddr,uint32_t n)
{
	for(;n!=0;n--)
	{
		*pBuffer++=*(volatile uint32_t*)(Bank5_SDRAM_ADDR+ReadAddr);
		ReadAddr++;
	}
}
...
//__attribute__ (( section (".sdram") ))  uint16_t testsram[250000]  
//uint16_t testsram[250000] __attribute__((section(".sdram.__at_0XC0000000")));
//uint16_t testsram[250000]  __attribute__((section(".SDRAM.__at_0XC0000000")));
//SDRAM内存测试	    
...
{  
	...
	{	
		...
		//LCD_ShowxNum(x+15*8,y,(uint16_t)(temp-sval+1)*16,5,16,0);	//显示内存容量  
