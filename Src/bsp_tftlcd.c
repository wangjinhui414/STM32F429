...
{   	 
	*(volatile uint16_t*)(0x60000000) =  regval;
}
//写LCD数据
//data:要写入的值
...
{
	return *(volatile uint16_t*)(0x60080000);
}					   
//写寄存器
//LCD_Reg:寄存器地址
//LCD_RegValue:要写入的数据
...
{	
	*(volatile uint16_t*)(0x60000000)  = LCD_Reg;		//写入要写的寄存器序号	 
	*(volatile uint16_t*)(0x60080000) = LCD_RegValue;//写入数据	    		 
}
//开始写GRAM
void LCD_WriteRAM_Prepare(void)
{
 	*(volatile uint16_t*)(0x60000000) = lcddev.wramcmd;	  
}	 
